...
let nestest () =
  ...
  while !continue do
    ...
    Alcotest.(check int) "Same cycle" cycleNb
      ((SCpu.cycle_count cpu * 3) mod 341);
    Alcotest.(check int) "Same accumulator" correctA
      (Uint8.to_int @@ get_reg cpu `A);
    Alcotest.(check int) "Same PC" correctPC
      (Uint16.to_int @@ get_pc cpu);
    Alcotest.(check int) "Same P register" correctP
      (Uint8.to_int @@ get_reg cpu `P);
    last_line := toParse ;
    SCpu.next_cycle cpu
  done ;
  Alcotest.(check int) "Final status 1" 0
    (Uint8.to_int @@ read_mem cpu 2);
...
let test_rom path () =
  ...
  let outStr = if outStr = "Passed" then outStr else
      String.init (end_pos - 0x6004 + 1)
        (fun i -> 
...
let test_alco = 
  ...
  run "6502-ml" [
    ...
    "misc", [
      test_rom "Special instructions" "test_roms/instr_test/16-special.nes.bin" ;
      test_rom "Absolute X wrap" "test_roms/instr_misc/01-abs_x_wrap.nes.bin" ;
